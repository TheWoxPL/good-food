        ...
        {restaurants.map((restaurant, index) => (
          <Card key={index} className="mb-4">
            <CardContent className="p-4 flex justify-between items-center">
              ...
              <Button variant="default" className="ml-5" onClick={() => handleSelect(restaurant)}>Select</Button>
            </CardContent>
          </Card>
        ))}
      </div>
...
{selectedRestaurant && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
    <div className="bg-white p-6 rounded-lg w-full max-w-lg">
      <h2 className="text-xl font-semibold mb-4">{selectedRestaurant.name} - Food List</h2>
      <ul className="mb-4">
        {selectedRestaurant.foodList.map((food, index) => (
          <li
            ...
            <span className="ml-4">${food.price.toFixed(2)}</span>
            <Button variant="default" className="ml-4 rounded-lg">+</Button>
          </li>
        ))}
      </ul>
